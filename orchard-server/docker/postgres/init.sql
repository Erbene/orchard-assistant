...
-- Core domain
-- ---------------------------------------------------------------------------
-- Irrigation zones live in Rachio (read live). `tree.zone_id` is a free-text
-- Rachio zone id. The local ``zone`` table is only a grower-facing label
-- overlay — Rachio has no equivalent field.
...
-- `tree.zone_id` is a free-text reference to a Rachio zone id.
...
CREATE TABLE IF NOT EXISTS task_templates (
    ...
    blocks              JSONB NOT NULL DEFAULT '[]'::jsonb,   -- list[{category, min_gap_days}]
...
-- Immutable snapshot of completed/skipped work (survives care-plan regen).
CREATE TABLE IF NOT EXISTS executed_task_log (
    id                  BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    tree_id             BIGINT NOT NULL REFERENCES tree(tree_id) ON DELETE CASCADE,
    template_id         BIGINT REFERENCES task_templates(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_executed_task_log_tree_executed
    ON executed_task_log (tree_id, executed_at DESC);
CREATE INDEX IF NOT EXISTS idx_executed_task_log_executed
    ON executed_task_log (executed_at DESC);
